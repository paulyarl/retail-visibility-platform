...
- **Entry source attribution:** `StoreViewTracker` also stamps `context.entry_source` from `?utm_source=` (verbatim) / `?source=` (e.g. `qr`) / `'shelf'` when `?shelf=` is present. A QR code printed for a listing encodes that listing's URL with `?source=qr`, so the resulting entry view is attributed to **QR as an entry source** — while the scan itself remains authoritative in `qr_scan_events` (channel metrics). The readout exposes `entrySources` (the **Entry Sources** section); organic/direct views are unattributed and not listed.
