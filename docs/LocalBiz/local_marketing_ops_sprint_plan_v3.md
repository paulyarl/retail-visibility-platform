...
| **Deliverable Generation** | Branded, formatted document creation from AI outputs (preview + paid) |
...
| **Reuse infrastructure** | File storage, database, caching, document generation, and deployment pipelines use existing services |
...
- Background job queue (for async prompt execution and document generation)
- Document/PDF generation capability (or ability to integrate one)
...
```sql
CREATE TABLE marketing_deliverable_templates (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(100) NOT NULL, -- "Review Response Pack", "Service Menu", "GBP Audit Report"
    deliverable_type VARCHAR(50) NOT NULL, -- review_responses, service_menu, gbp_audit, testimonial_cards
    category VARCHAR(100), -- NULL = all categories
    version INTEGER DEFAULT 1,
    html_template TEXT NOT NULL, -- HTML template with {{variable}} placeholders
    css_overrides TEXT, -- optional category-specific styling
    header_content TEXT, -- default header text/branding
    footer_content TEXT, -- default footer text/disclaimer
...
CREATE TABLE marketing_deliverables (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    campaign_id UUID REFERENCES marketing_campaigns(id) ON DELETE CASCADE,
    execution_id UUID REFERENCES marketing_prompt_executions(id), -- links to the AI output used
    template_id UUID REFERENCES marketing_deliverable_templates(id),
    ...
    sent_at TIMESTAMP, -- when it was emailed/shown to the business
...
|------|-------|-------------|
| Map existing platform tech stack, auth, UI patterns, and document generation | Tech Lead | Compatibility matrix |
| Evaluate document generation options (existing vs. new library) | Tech Lead | Decision doc |
| Finalize database schema and generate migrations | Backend Dev | Migration files + ERD |
| Design UI component specs (forms, tables, kanban, modals, document preview) | UX/UI Designer | Figma prototypes |
...
- [ ] Document generation approach selected and prototyped
...
### Sprint 3: Deliverable Generation + Dashboard + Direct AI Integration + Filter Review (Week 5–6)
**Goal:** In-platform document generation from AI outputs, real-time dashboard, direct AI execution, and quality filter review queue.

| Task | Owner | Points |
|------|-------|--------|
| Build Deliverable Template CRUD (HTML templates with variable system) | Backend Dev | 5 |
| Build document generation engine (HTML template → PDF) | Backend Dev | 8 |
| Build branding injection system (logo, colors, operator name) | Backend Dev | 3 |
| Build preview vs. paid watermark logic | Backend Dev | 2 |
| Build Deliverable Preview UI (side-by-side: raw output vs. formatted document) | Frontend Dev | 5 |
| Build Deliverable Generation flow (select template → inject content → generate → preview → save) | Frontend Dev | 5 |
...
- [ ] Deliverable generation produces branded PDF from AI output
- [ ] Preview deliverables show watermark; paid deliverables do not
...
| Build deliverable template seed script (default PDF templates) | Backend Dev | 2 |
...
- [ ] Default prompt and deliverable templates seeded and active
...
## 7. Deliverable Generation Module (Detailed)

### 7.1 Purpose

The Deliverable Generation Module transforms raw AI outputs into **branded, formatted, client-ready documents**. This is the bridge between "AI wrote text" and "business owner receives a professional PDF."

### 7.2 Deliverable Types

| Type | Description | Output Format |
|------|-------------|---------------|
| **Review Response Pack** | Numbered list of responses + reusable templates | PDF |
| **Service Menu** | Designed one-page menu with pricing tiers | PDF |
| **GBP Audit Report** | Scorecard with competitor comparison | PDF |
| **Testimonial Graphics** | Quote cards with branding | PDF or PNG set |
| **NAP Consistency Report** | Comparison table across platforms | PDF |
| **Local SEO Content Pack** | Service-area pages with meta data | PDF or DOCX |
| **Lead Magnet** | Designed PDF guide or checklist | PDF |

### 7.3 Document Generation Flow

```
┌─────────────────┐     ┌─────────────────────┐     ┌─────────────────────┐     ┌─────────────────┐
│  AI OUTPUT      │───→ │  SELECT TEMPLATE    │───→ │  INJECT CONTENT     │───→ │  GENERATE PDF   │
│  (approved text)│     │  (deliverable type) │     │  + BRANDING         │     │  (preview/paid) │
└─────────────────┘     └─────────────────────┘     └─────────────────────┘     └─────────────────┘
                                                                                         │
                                                                                         │
                                                                              ┌─────────────────┐
...
Deliverable templates use **HTML + CSS** with variable injection:

```html
<!-- Example: Review Response Pack Template -->
<!DOCTYPE html>
<html>
<head>
  <style>
    body { font-family: {{font_family}}; color: {{text_color}}; }
    .header { background: {{primary_color}}; padding: 24px; }
    .response { margin-bottom: 20px; border-left: 3px solid {{accent_color}}; padding-left: 16px; }
    .reviewer-name { font-weight: 500; color: {{primary_color}}; }
    .watermark { opacity: 0.15; position: fixed; top: 40%; left: 30%; font-size: 48px; transform: rotate(-30deg); }
  </style>
</head>
<body>
  <div class="header">
    <h1>Review Response Package</h1>
    <p>Prepared for {{business_name}} | {{city}}</p>
  </div>
  ...
  <div class="content">
    {{#each responses}}
    <div class="response">
      <div class="reviewer-name">{{reviewer_name}} — {{stars}} stars</div>
      <p>{{response_text}}</p>
    </div>
    {{/each}}
  </div>
  ...
  <div class="footer">
    <p>{{operator_name}} | Local Marketing Operations</p>
...
Per-campaign or per-operator branding settings:

| Setting | Description | Default |
|---------|-------------|---------|
| `operator_name` | Name shown in header/footer | Platform admin name |
| `operator_logo_url` | Logo in header | Platform logo |
| `primary_color` | Header background, accents | `#111827` |
| `accent_color` | Borders, highlights | `#3B82F6` |
| `text_color` | Body text | `#1F2937` |
| `font_family` | Typography | System sans-serif |
| `footer_disclaimer` | Legal/footer text | "This is a preview. Full package delivered upon payment." |

### 7.6 Preview vs. Paid Logic

| Aspect | Preview | Paid |
|--------|---------|------|
| Watermark | "PREVIEW" diagonal watermark | None |
| Response count | 5–10 sample responses | All responses |
| Templates included | No | Yes (reusable templates) |
| Branding | Operator branding | Operator + business co-branding |
| File naming | `{ID}_preview_{DATE}.pdf` | `{ID}_paid_{TIER}_{DATE}.pdf` |
| Expiry | Optional: preview link expires in 7 days | Permanent |

### 7.7 Document Generation Engine

**Recommended approach (adapt to existing platform):**

| Option | Implementation | Best For |
|--------|---------------|----------|
| **Puppeteer + HTML** | Node service renders HTML template to PDF | Full control, custom fonts, complex layouts |
| **wkhtmltopdf** | Command-line HTML-to-PDF | Simple, fast, lightweight |
| **Gotenberg** | Docker-based HTML-to-PDF API | Scalable, microservice-friendly |
| **Existing platform doc gen** | Reuse current invoice/report generator | Fastest integration, least risk |
| **Cloud API** (DocRaptor, PDFShift) | External API call | Zero infrastructure, per-document cost |

**Recommendation:** If the existing platform already generates invoices, reports, or PDFs, **extend that system** first. If not, Puppeteer or Gotenberg running as a background job is the most flexible path.

### 7.8 Deliverable UI Screens

| Screen | Purpose |
|--------|---------|
| **Deliverable Template Library** | CRUD for HTML templates, preview rendered output |
| **Campaign Deliverables Panel** | List all generated deliverables for a campaign, download, send |
| **Generate Deliverable Flow** | Select template → preview raw content → apply branding → generate → preview PDF → confirm |
| **Deliverable Preview Modal** | Side-by-side: source text (left) + rendered PDF (right) |
| **Send Deliverable Modal** | Choose method (email, download, shareable link), add note, confirm send |
...
| **One-click actions** | Stage transitions, file downloads, document generation, and quick edits require minimal clicks |
| **Contextual prompts** | Prompt workspace and deliverable generation accessible from campaign detail; no page jumps |
...
| **Campaign Detail** | View full journey + execute prompts + generate deliverables | Edit record, upload files, change stage, view timeline, run prompts, generate PDF |
...
| **Deliverable Generator** | Create branded PDF from AI output | Select template, apply branding, preview, watermark toggle |
| **Deliverable Preview** | Review rendered PDF before sending | Download, edit source, regenerate, send |
...
| **Deliverable Template Library** | Manage document templates | Create, edit, preview HTML templates, set defaults |
...
| `DeliverableGenerator` | Campaign detail | Template selector, branding panel, generate button |
| `DeliverablePreview` | Modal/panel | PDF viewer, source text toggle, watermark badge |
...
| Existing deliverables (PDFs) | `marketing_deliverables` | Upload → create records with status mapping | Low |

### 9.2 Migration Script Outline
...
for _, row in campaigns_df.iterrows():
    ...
    # 6. Migrate existing deliverables
    deliverable_path = f"./legacy/{row['date_entered']}/{row['folder_name']}/deliverables/"
    migrate_deliverables(campaign.id, deliverable_path)
...
| **Document Generation** | Visual regression on PDF output | All deliverable templates |

### 10.2 Critical User Journeys (CUJs)
...
4. Admin selects deliverable template → generates branded PDF → previews → saves
5. Admin marks deliverable as "preview" → sends to business → marks campaign "Shown"
6. Admin marks campaign "Paid" → generates paid deliverable (no watermark) → sends
7. Admin marks campaign "Delivered" → pitches retainer
8. Admin submits daily scorecard → views updated dashboard metrics
9. Admin filters tracker by category + stage → exports CSV
10. Admin edits prompt template → creates new version → sets as default
11. Admin edits deliverable template → previews HTML → publishes
...
| Deliverable generation time | N/A (manual formatting) | <2 min | `marketing_deliverables` generation timestamp |
...
| Deliverable send rate | N/A | 100% | All paid campaigns have associated deliverable |
...
| Document generation fails or slow | Medium | High | Background job queue, fallback to copy-paste, monitoring |

---

## 13. v1.1 Roadmap (Post-MVP)

| Feature | Value | Effort |
|---------|-------|--------|
| **Batch prompt execution** — Run fulfill across 5 campaigns simultaneously | High | Medium |
| **Batch deliverable generation** — Generate preview PDFs for 5 campaigns at once | High | Medium |
...
| **Client portal** — Businesses view their audit results and deliverables (read-only) | Medium | High |
...
| **Custom deliverable builder** — Drag-and-drop WYSIWYG for non-technical admins | Medium | High |
...
| Deliverable template | `{TYPE}_template_v{VERSION}.html` |
...
### D. Deliverable Template Variable Reference

| Variable | Source | Example |
|----------|--------|---------|
| `{{business_name}}` | `marketing_campaigns.business_name` | Austin Shine Auto Detailing |
| `{{city}}` | `marketing_campaigns.city` | Austin, TX |
| `{{category}}` | `marketing_campaigns.category` | Auto Detailing |
| `{{operator_name}}` | Current user profile | Local Marketing Ops |
| `{{primary_color}}` | Branding config | `#111827` |
| `{{accent_color}}` | Branding config | `#3B82F6` |
| `{{is_preview}}` | Deliverable status | `true` or `false` |
| `{{date}}` | Generation date | July 28, 2026 |
| `{{responses}}` | AI output array | Iterated list |
| `{{footer_disclaimer}}` | Template config | "Preview only. Full package upon payment." |
...
*End of Sprint Plan Document v3.0*
