# Local Category Market Opportunity Audit | Seek: Category Audit V1 (Signal-Aligned Variant)
...
National and regional chains may be included, but label ownership type.
...
## Per-Business Detected Audit Signals (Triage Engine Mapping)

For every business in `sampled_businesses` and `top_competitors`, evaluate the observable digital presence and populate its `detected_signals` string array. Use **ONLY** the exact string codes defined below when verified public evidence supports the signal:
