...
- **Location axis:** `/place` home renders `PlacesIndexHero` (server component — breadcrumb, title, browse copy, search entry, shelf counts) first, then `PlaceNationalPanel` (server component) as page copy under the title — the national packet's `bodyCopy`/`effective.description` intro + measured coverage band (`context.national_coverage`: listings / markets / states) + top-market chips linking `/place/city/{city-slug}` (the seed city shelves — real destinations, no phantom state pages). The panel originally rendered *above* the client index, which put the header under the copy (fixed 2026-09-23); it must stay below the hero. `generateMetadata` is now packet-driven (national `metaTitle`/`description` with static fallback). Panel renders nothing when no national packet exists — graceful-absent.
- **Category axis:** `page.tsx` fetches the roster server-side and passes it to `PlacesIndexClient` — each category card surfaces `context.category_overview` (fallback `effective.description`) as a 2-line excerpt, keyed on `categoryKey`/`categoryName`. The category shelf itself is read server-side too (`getCategories`): the hero carries its counts and `PlacesIndexClient` takes it as `initialCategories` (client fetch retained only as the fallback when the server read failed). Same treatment on `/directory/categories` (`AllCategoriesClient` + roster prop from its `page.tsx`).
