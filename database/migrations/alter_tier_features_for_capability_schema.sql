-- Alter Existing Tier Features Table for Tier-Specific Capability Schema
-- This modifies the existing tier_features_list table to work with the new architecture

-- First, create the new tables for the capability schema
...
-- 4. ALTER the existing tier_features_list table to work with capability types
-- First, check if the new columns exist before adding them
DO $$
BEGIN
    -- Add capability_type_id column if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='tier_features_list' AND column_name='capability_type_id'
    ) THEN
        ALTER TABLE tier_features_list ADD COLUMN capability_type_id TEXT REFERENCES capability_type_list(id) ON DELETE CASCADE;
    END IF;

    -- Add tier_specific_restrictions column if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='tier_features_list' AND column_name='tier_specific_restrictions'
    ) THEN
        ALTER TABLE tier_features_list ADD COLUMN tier_specific_restrictions JSONB;
    END IF;

    -- Add marketing_name column if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='tier_features_list' AND column_name='marketing_name'
    ) THEN
        ALTER TABLE tier_features_list ADD COLUMN marketing_name TEXT;
    END IF;

    -- Add created_by column if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='tier_features_list' AND column_name='created_by'
    ) THEN
        ALTER TABLE tier_features_list ADD COLUMN created_by TEXT;
    END IF;

    -- Add updated_by column if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='tier_features_list' AND column_name='updated_by'
    ) THEN
        ALTER TABLE tier_features_list ADD COLUMN updated_by TEXT;
    END IF;
END $$;
...
-- 9. Assign tier-specific capabilities to tiers (update existing tier_features_list)
DO $$
DECLARE
    discovery_tier_id TEXT;
    starter_tier_id TEXT;
...
BEGIN
    ...
    -- Clear existing tier assignments for product types
    DELETE FROM tier_features_list WHERE feature_key LIKE '%_product_type';
    
    -- Assign each tier their specific product type capability
    -- Note: Using existing table structure with feature_key and feature_name
    -- We'll use capability type key as feature_key for now
    INSERT INTO tier_features_list (id, tier_id, feature_key, feature_name, is_enabled, is_highlighted, highlight_order, marketing_name) VALUES
    (gen_random_uuid()::text, discovery_tier_id, 'discovery_product_type', 'Discovery Product Types', true, true, 1, 'Basic Product Types'),
    (gen_random_uuid()::text, starter_tier_id, 'starter_product_type', 'Starter Product Types', true, true, 1, 'Enhanced Product Types'),
    (gen_random_uuid()::text, storefront_tier_id, 'storefront_product_type', 'Storefront Product Types', true, true, 1, 'Advanced Product Types'),
    (gen_random_uuid()::text, professional_tier_id, 'professional_product_type', 'Professional Product Types', true, true, 1, 'Professional Product Types'),
    (gen_random_uuid()::text, enterprise_tier_id, 'enterprise_product_type', 'Enterprise Product Types', true, true, 1, 'Unlimited Product Types');
...
SELECT 
    ...
    tfl.feature_key as capability_type_key,
    tfl.feature_name as capability_type_name,
    'product_types' as capability_category,
    tfl.is_enabled as capability_enabled,
    tfl.is_highlighted,
    tfl.highlight_order,
    tfl.highlight_description,
    tfl.marketing_name as capability_marketing_name,
    tfl.metadata as tier_specific_restrictions,
    ...
    (
        SELECT JSON_AGG(
            JSON_BUILD_OBJECT(
                ...
                'effective_restrictions', COALESCE(tfl.metadata, cfl.restrictions)
            ) ORDER BY fl.sort_order
        )
        FROM capability_features_list cfl
        JOIN features_list fl ON cfl.feature_id = fl.id
        WHERE cfl.capability_type_id = (SELECT id FROM capability_type_list WHERE key = tfl.feature_key) 
          AND fl.is_active = true AND cfl.is_active = true
    ) as features,
...
WHERE stl.is_active = true
  ...
  AND tfl.feature_key LIKE '%_product_type'
...
SELECT 
    ...
    stl.sort_order as tier_sort_order,
...
GROUP BY ctl.id, ctl.key, ctl.name, ctl.category, stl.tier_key, stl.display_name, ctl.description, ctl.sort_order, stl.sort_order
