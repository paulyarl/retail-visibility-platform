...
set -e
echo "🔧 RECREATING storefront_category_counts"
echo "========================================="
...
SELECT
  ...
  COUNT(CASE WHEN ii.description IS NOT NULL AND ii.description <> '' THEN 1 ELSE NULL::INTEGER END) AS products_with_descriptions,
  COUNT(CASE WHEN ii.brand IS NOT NULL AND ii.brand <> 'Unknown' THEN 1 ELSE NULL::INTEGER END) AS products_with_brand,
  ...
  COALESCE(srs.rating_avg, 0) AS store_rating_avg,
...
GROUP BY
  ii.tenant_id, t.name, dc.id, dc.name, dc.slug, dc.\"googleCategoryId\", dc.\"sortOrder\", dc.\"parentId\",
  srs.rating_avg, srs.rating_count, srs.rating_1_count, srs.rating_2_count, srs.rating_3_count,
  srs.rating_4_count, srs.rating_5_count, srs.verified_purchase_count, srs.last_review_at
HAVING COUNT(ii.id) > 0;
"

echo ""
echo "📊 Adding unique index..."
doppler run -- psql "$PRODUCTION_DATABASE_URL" -c "CREATE UNIQUE INDEX storefront_category_counts_unique_idx ON storefront_category_counts (tenant_id, category_id);"
...
doppler run -- psql "$PRODUCTION_DATABASE_URL" -c "SELECT column_name FROM information_schema.columns WHERE table_name = 'storefront_category_counts' AND column_name = 'category_id';"
