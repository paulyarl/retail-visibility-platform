...
set -e
echo "🔧 RECREATING storefront_category_counts WITH CORRECT STRUCTURE"
echo "================================================================"
...
echo ""
echo "📊 Creating storefront_category_counts with staging structure..."
doppler run -- psql "$PRODUCTION_DATABASE_URL" "
...
echo ""
echo "📊 Verifying structure - checking for category_id column..."
doppler run -- psql "$PRODUCTION_DATABASE_URL" -c "
...
echo "📊 Sample data..."
doppler run -- psql "$PRODUCTION_DATABASE_URL" -c "
SELECT tenant_id, category_id, category_name, product_count 
FROM storefront_category_counts 
LIMIT 5;
"

echo ""
echo "🎯 storefront_category_counts recreated!"
