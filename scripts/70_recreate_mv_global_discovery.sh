...
set -e
echo "🔧 RECREATING mv_global_discovery WITH product_slug"
echo "=================================================="
...
echo ""
echo "📊 Creating mv_global_discovery with product_slug..."
doppler run -- psql "$PRODUCTION_DATABASE_URL" -c "
...
echo ""
echo "📊 Verifying product_slug column exists..."
doppler run -- psql "$PRODUCTION_DATABASE_URL" -c "
SELECT column_name 
FROM information_schema.columns 
WHERE table_name = 'mv_global_discovery' 
  AND column_name IN ('product_slug', 'platform_tenant_count', 'platform_purchase_count', 'platform_total_stock');
"

echo ""
echo "📊 Sample data with product_slug..."
doppler run -- psql "$PRODUCTION_DATABASE_URL" -c "
SELECT inventory_item_id, product_slug, brand_normalized, category_normalized, platform_tenant_count 
...
echo ""
echo "🎯 mv_global_discovery recreated with product_slug!"
