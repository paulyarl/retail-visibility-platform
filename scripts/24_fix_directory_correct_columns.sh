...
echo "🔧 Fix Directory Views with Correct Columns"
echo "=========================================="
...
echo "📋 Creating directory views with correct column names..."

echo "📦 Creating directory_category_listings (with correct columns)..."
...
GROUP BY t.id, t.name, t.slug, t.subscription_tier, t.location_status, t.directory_visible, t.updated_at, ii.category_path[1];
"

echo "📦 Creating directory_home_summary_mv (with correct columns)..."
psql "$PRODUCTION_DATABASE_URL" -c "
...
SELECT 
    ...
    COUNT(DISTINCT ii.category_path[1]) as total_categories,
...
echo "📦 Creating storefront_products (with correct columns)..."
psql "$PRODUCTION_DATABASE_URL" -c "
...
SELECT 
    ...
    ii.sale_price_cents,
    COALESCE(ii.category_path[1], 'uncategorized') as category,
    COALESCE(ii.category_path[2], '') as subcategory,
    ii.brand,
    ii.image_url,
    ii.quantity as quantity_available,
    ii.item_status as is_active,
    ii.created_at,
    ii.updated_at,
    t.name as tenant_name,
    t.slug as tenant_slug,
...
echo "📦 Creating storefront_category_counts (with correct columns)..."
psql "$PRODUCTION_DATABASE_URL" -c "
...
WHERE t.location_status = 'active'
  AND ii.item_status = 'active'
GROUP BY ii.tenant_id, t.name, t.slug, ii.category_path[1];
"

echo "✅ All directory views created with correct columns"
...
echo ""
echo "📊 Checking inventory items status..."

# Check inventory items status
echo "🔍 Inventory items by status:"
psql "$PRODUCTION_DATABASE_URL" -c "
...
echo ""
echo "🎯 Directory Views Fixed!"
echo "======================"
echo ""
echo "✅ What was fixed:"
echo "   - Used category_path[1] instead of category"
echo "   - Used item_status instead of is_active"
echo "   - Used quantity instead of quantity_available"
echo "   - All directory views created with correct columns"
echo ""
...
echo "   - Active inventory items: $(psql "$PRODUCTION_DATABASE_URL" -t -c "SELECT COUNT(*) FROM inventory_items WHERE item_status = 'active';" | tr -d ' ')"
