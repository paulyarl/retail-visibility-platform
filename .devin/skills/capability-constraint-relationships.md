...
- **Forgetting to update `CONSTRAINT_METADATA`** — The `CONSTRAINT_METADATA` constant in `apps/api/src/routes/admin/capability-constraints.ts` is a **static hardcoded list** of capabilities, fields, operators, and values used to populate the constraint form dropdowns. When a new capability is added to the resolver architecture, you MUST add a corresponding entry to `CONSTRAINT_METADATA` with the new capability's key, label, fields (with correct `value_type`, `operators`, and `values`), derived from the new `EffectiveXxx` interface in `types.ts`. Otherwise the new capability won't appear in the Source/Target dropdowns and admins won't be able to create constraints referencing it.
