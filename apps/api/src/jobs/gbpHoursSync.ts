...
export async function runGbpHoursSync(payload: GbpHoursPayload) {
  ...
  const hours = await prisma.business_hours_list.upsert({
  ...
  const specials = await prisma.business_hours_special_list.findMany({ where: { tenant_id: tenantId } })
  ...
  for (let i = 0; i < 5; i++) {
    try {
      ...
      await prisma.business_hours_list.update({
        where: { tenant_id: tenantId },
  ...
  await prisma.business_hours_list.update({ where: { tenant_id: tenantId }, data: { sync_attempts: attempt, last_error: lastError, updated_at: new Date() } })
  try {
