...
router.get('/tenant/:tenantId/business-hours',
  ...
  const row = await prisma.business_hours.findUnique({ where: { tenantId } })
...
router.get('/tenant/:tenantId/business-hours/special',
  ...
  async (req, res) => {
  const { tenantId } = req.params
  const rows = await prisma.business_hours_special.findMany({ where: { tenantId }, orderBy: { date: 'asc' } })
...
router.put('/tenant/:tenantId/business-hours/special',
  ...
  for (const o of overrides) {
    ...
    await prisma.business_hours_special.upsert({
