...
router.get('/:id/directory/listing', authenticateToken, checkTenantAccess, async (req: Request, res: Response) => {
  try {
    ...
    let settings = await prisma.directory_settings.findUnique({
      where: { tenantId },
    });
    ...
    if (!settings) {
      ...
      const tenant = await prisma.tenant.findUnique({
        where: { id: tenantId },
        include: { tenant_business_profile: true },
      });
      ...
      let slug = baseSlug;
      let counter = 1;
      while (await prisma.directory_settings.findUnique({ where: { slug } })) {
        slug = `${baseSlug}-${counter}`;
        counter++;
      }

      settings = await prisma.directory_settings.create({
        data: {
          tenantId,
          slug,
          isPublished: false,
