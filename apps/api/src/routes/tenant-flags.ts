...
router.get('/tenant-flags/:tenantId', checkTenantAccess, async (req, res) => {
  ...
  const tenantFlags = await prisma.tenant_feature_flags_list.findMany({ 
  ...
  const platformFlags = await prisma.platform_feature_flags_list.findMany({
    where: { allow_tenant_override: true },
    orderBy: { flag: 'asc' }
  })
  
  // Merge: show platform flags with override permission, use tenant flag if exists
  const tenantFlagMap = new Map(tenantFlags.map((f: any) => [f.flag, f]))
  
  const merged = platformFlags.map((pf: any) => {
...
router.put('/tenant-flags/:tenantId/:flag', requireTenantOwner, async (req, res) => {
  ...
  const row = await prisma.tenant_feature_flags_list.upsert({
