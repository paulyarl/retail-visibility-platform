...
// GET /api/public/tenants/:tenantId/reviews/summary - Get rating summary for a tenant (public)
router.get('/summary', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error fetching public tenant review summary:', undefined, { error: { name: (error as any)?.name || 'Error', message: (error as any)?.message || String(error), stack: (error as any)?.stack } });
    res.status(500).json({
      success: false,
...
// GET /api/public/tenants/:tenantId/reviews - Get approved reviews for a tenant (public)
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const formattedReviews = reviewsResult.rows.map((review: any) => ({
      id: review.id,
    ...
    logger.error('Error fetching public tenant reviews:', undefined, { error: { name: (error as any)?.name || 'Error', message: (error as any)?.message || String(error), stack: (error as any)?.stack } });
    res.status(500).json({
      success: false,
...
// GET /api/public/tenants/:tenantId/reviews/approved - Get approved reviews (alias for consistency)
router.get('/approved', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error fetching approved public tenant reviews:', undefined, { error: { name: (error as any)?.name || 'Error', message: (error as any)?.message || String(error), stack: (error as any)?.stack } });
    res.status(500).json({
      success: false,
