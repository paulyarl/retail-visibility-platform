...
router.get('/tenants/:tenantId/orders/:orderId', authenticateToken, async (req, res) => {
  try {
    ...
    const orderDetail = {
      ...
      // Tenant contact info from metadata
      tenantAddress: tenantMetadata.address || null,
      tenantCity: tenantMetadata.city || null,
      tenantState: tenantMetadata.state || null,
      tenantPostalCode: tenantMetadata.postal_code || null,
      tenantCountry: tenantMetadata.country || null,
      tenantPhone: tenantMetadata.phone || null,
      tenantEmail: tenantMetadata.email || null,
      customerName: order.customer_name,
      customerEmail: order.customer_email,
      customerPhone: order.customer_phone,
