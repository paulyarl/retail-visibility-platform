...
export function formatEnrichmentCategoryVocabulary(
  ...
  const lines: string[] = [
    ...
    `the ${directory.length} canonical category labels below are the storefront`,
    'categories those retailers are shelved under, each a public directory',
    'page, not an abstract taxonomy node. Non-shelf businesses (restaurants,',
    'services, online-only sellers) have no canonical label — emitting one',
    'is legitimate when no shelf fits; it simply cannot hot-link.',
    '',
    'Every related-category field you emit renders as a chip that hot-links',
    'to a shelf ONLY when the name matches a shelf label exactly (trimmed,',
    'case-insensitive). An invented or near-miss name renders as dead plain',
    'text — alignment with this list is what makes the link seamless.',
