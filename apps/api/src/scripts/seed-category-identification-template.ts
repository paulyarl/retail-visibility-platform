...
=== KNOWN CATEGORIES ===
Where a KNOWN CATEGORY VOCABULARY block is present in this prompt, match the business against its lists (KNOWN CATEGORIES, ENRICHED CATEGORIES, REGISTERED LABELS) and set is_known_category accordingly. Where the block is absent, fall back to general knowledge of common category labels.
...
The platform exists to make the physical shelves of independent brick-and-mortar retailers visible to customers who walk through the door. A category label is not a positioning statement — it becomes a public directory page that hosts every similar business in the market, and your candidate list propagates ONE business across shelves: the primary candidate becomes its canonical shelf and every accepted secondary files it on another public shelf. Evaluate every candidate as a directory shelf:
