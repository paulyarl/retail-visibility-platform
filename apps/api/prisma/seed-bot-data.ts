...
const INTENTS = [
  ...
  {
    name: 'support.ticket_status',
    category: 'support',
    description: 'User wants to check the status of a support ticket',
    examples: [
      'check my ticket',
      'ticket status',
      'where is my support request',
      'has my ticket been answered',
      'support ticket update',
      'my open tickets',
    ],
    confidence_threshold: 0.3,
    mapped_skill: 'crm-assistant',
  },
  {
  ...
  {
    name: 'support.inquiry_check',
    category: 'support',
    description: 'User wants to check on an inquiry they submitted',
    examples: [
      'check my inquiry',
  ...
  },
  {
    name: 'policy.inquiry',
    category: 'policy',
    description: 'User asks about return, shipping, refund, privacy, or terms policies',
    examples: [
...
const SKILLS = [
  ...
  },
  {
    name: 'crm-assistant',
    version: '1.0.0',
    description: 'CRM-aware assistant: create support tickets, look up ticket status, inject CRM context',
    endpoint: '/api/public/bot/crm/ticket-status',
    required_capabilities: ['chatbot_skill_crm_assistant'],
    tier_gates: ['professional', 'enterprise', 'organization', 'chain_starter', 'chain_professional', 'chain_enterprise'],
    capability_gates: ['chatbot_skills_enabled'],
    tenant_status_gates: ['active', 'trialing'],
    featured_aware: false,
    refresh_cadence_minutes: 5,
    ...
    skill_card_schema: {
      type: 'ticket_status',
      fields: ['ticket_id', 'title', 'status', 'priority', 'last_updated'],
      max_results: 5,
    },
    default_config: { include_conversation_context: true },
  },
  {
    name: 'policy-faq',
    version: '1.0.0',
    description: 'Answer customer questions about store policies (returns, shipping, refunds, privacy, terms)',
    endpoint: '/api/public/bot/policies',
    required_capabilities: ['chatbot_skill_policy_faq'],
    tier_gates: ['commitment', 'ecommerce', 'omnichannel', 'professional', 'enterprise', 'organization', 'chain_starter', 'chain_professional', 'chain_enterprise'],
    capability_gates: ['chatbot_skills_enabled'],
    tenant_status_gates: ['active', 'trialing'],
    featured_aware: false,
    ...
    skill_card_schema: {
      type: 'policy_text',
      fields: ['policy_type', 'content', 'last_updated'],
    },
    default_config: { include_all_policies: false },
  },
];
