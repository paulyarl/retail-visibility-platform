import * as Sentry from '@sentry/nextjs';

// Completely disable Sentry in development to avoid deprecation warnings
if (process.env.NODE_ENV !== 'development') {
  Sentry.init({
    dsn: process.env.NEXT_PUBLIC_SENTRY_DSN,
    
    // Set tracesSampleRate to 1.0 to capture 100% of transactions for performance monitoring.
    // We recommend adjusting this value in production
    tracesSampleRate: 0.1,
    
    // Note: if you want to override the automatic release value, do not set a
    // `release` value here - use the environment variable `SENTRY_RELEASE`, so
    // that it will also get attached to your source maps
    
    // Use custom environment variable to distinguish staging from production
    environment: process.env.NEXT_PUBLIC_SENTRY_ENVIRONMENT || process.env.NODE_ENV,
    
    // Server-side error filtering
    beforeSend(event, hint) {
      // Don't send errors from development (this check is redundant now but kept for safety)
      if (process.env.NODE_ENV === 'development') {
        console.error('Sentry error (not sent in dev):', hint.originalException || event);
        return null;
      }
      return event;
    },
  });
}
