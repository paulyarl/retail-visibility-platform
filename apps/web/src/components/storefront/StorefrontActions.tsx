...
import { Heart, Printer, Share2, MessageSquare, ShoppingCart, ArrowLeft } from 'lucide-react';
...
export default function StorefrontActions({ 
  ...
  useEffect(() => {
    const favorites = JSON.parse(localStorage.getItem('storefront_favorites') || '{}');
    setFavorited(!!favorites[tenantId]);
    
    // Load favorite count (mock for now - could be from API)
    const counts = JSON.parse(localStorage.getItem('storefront_favorite_counts') || '{}');
    setFavoriteCount(counts[tenantId] || 0);
  }, [tenantId]);
  ...
  const handleFavorite = () => {
    ...
    if (favorited) {
      ...
      counts[tenantId] = Math.max(0, (counts[tenantId] || 0) - 1);
      setFavorited(false);
      setFavoriteCount(counts[tenantId]);
    } else {
      favorites[tenantId] = true;
      counts[tenantId] = (counts[tenantId] || 0) + 1;
      setFavorited(true);
      setFavoriteCount(counts[tenantId]);
    }
    
    localStorage.setItem('storefront_favorites', JSON.stringify(favorites));
    localStorage.setItem('storefront_favorite_counts', JSON.stringify(counts));
  };
  ...
  const shareLinks = {
    ...
    email: `mailto:?subject=${encodeURIComponent(businessName)}&body=${encodeURIComponent(`${shareText}\n\n${shareUrl}`)}`,
  };
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {showBackButton && (
        <button
          onClick={handleBack}
          className="inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium bg-gray-600 hover:bg-gray-700 text-white transition-colors"
          title="Go back"
        >
