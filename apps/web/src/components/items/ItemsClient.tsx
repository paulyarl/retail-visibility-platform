...
export default function ItemsClient({
  ...
  const [checkingScanSession, setCheckingScanSession] = useState(false);
  ...
  useEffect(() => {
    ...
    // Check for active scan session
    if (tenantId) {
      checkActiveScanSession();
    }
  }, [tenantId]);

  const checkActiveScanSession = async () => {
    try {
      setCheckingScanSession(true);
      const apiBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:4000';
      const res = await api.get(`${apiBaseUrl}/api/scan/sessions?tenantId=${tenantId}&status=active`);
      
      if (res.ok) {
        const data = await res.json();
        // If there's an active session, store it
        if (data.sessions && data.sessions.length > 0) {
          setActiveScanSession(data.sessions[0]);
        } else {
          setActiveScanSession(null);
        }
      }
    } catch (error) {
      console.error('[ItemsClient] Failed to check scan session:', error);
    } finally {
      setCheckingScanSession(false);
    }
  };
  ...
  useEffect(() => {
    // Refresh items when tenantId, page, pageSize, search, or filter changes
    if (tenantId) {
      refresh();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tenantId, currentPage, pageSize, q, statusFilter, visibilityFilter]);
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-900" suppressHydrationWarning>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          ...
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-lg p-4 shadow-md">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-700">
                    {activeScanSession.scannedCount || 0} items scanned • {activeScanSession.duplicateCount || 0} duplicates detected
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Button
                  variant="primary"
                  onClick={() => window.location.href = `/t/${tenantId}/scan/${activeScanSession.id}`}
                ...
                <Button
                  variant="secondary"
                  onClick={() => checkActiveScanSession()}
                  disabled={checkingScanSession}
                >
                ...
                {paginatedItems.map((i) => (
                  <div key={i.id} className={`p-4 border border-neutral-200 dark:border-neutral-700 rounded-lg hover:shadow-2xl hover:border-neutral-400 dark:hover:border-neutral-500 hover:-translate-y-1 transition-all duration-200 bg-white dark:bg-neutral-800 shadow-md ${viewMode === 'list' ? 'w-full' : ''}`}>
                    <div className="flex items-start gap-4 mb-3">
                      ...
                      {/* Google Sync Status with detailed reasons */}
                      <div className="mt-2">
                        <SyncStatusIndicator 
                    ...
                    <div className="flex flex-wrap items-center gap-1 pt-3 border-t border-neutral-100">
                      ...
                      <Button 
                        size="sm" 
                        variant="primary"
                        onClick={() => window.location.href = `/t/${tenantId}/scan?mode=enrich&itemId=${i.id}&sku=${encodeURIComponent(i.sku)}`}
        ...
        {/* Badge Legends - Moved after items for better UX */}
        {items.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {/* Status Badge Guide */}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h4 className="text-sm font-semibold text-purple-900 mb-2 flex items-center gap-2">
                    <span>✨ Scan to Enrich Guide</span>
                    <Badge className="bg-purple-100 text-purple-800 border-purple-200 text-xs">NEW</Badge>
                  </h4>
                  <div className="grid grid-cols-1 gap-2 text-xs text-purple-800">
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="primary" className="bg-gradient-to-r from-purple-600 to-pink-600 text-xs px-2 py-1 h-auto pointer-events-none">
                        Scan to Enrich
                      </Button>
                      <span>Click this button on any product to add enrichment data</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-purple-700 font-medium">📸 Images:</span>
                      <span>Automatically fetch product photos from barcode</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-purple-700 font-medium">📝 Details:</span>
                      <span>Get descriptions, specifications, and brand info</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-purple-700 font-medium">⚡ Fast:</span>
                      <span>Scan once, enrich instantly - saves hours of manual entry</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <CardContent>
              <div className="flex flex-wrap gap-3">
                ...
                <Button
                  ...
                  onClick={() => window.location.href = `/t/${tenantId}/scan`}
                  disabled={!tenantId}
                  title="Start scanning barcodes to add products"
                  className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700"
                >
                  <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" />
                  ...
                  Scan Barcodes
                  <span className="ml-2 text-xs bg-white/20 px-2 py-0.5 rounded font-semibold">✨ NEW</span>
                </Button>
              </div>
            </CardContent>
          </Card>
