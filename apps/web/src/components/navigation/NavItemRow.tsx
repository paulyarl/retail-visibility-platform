...
const IconComponents = {
  ...
  barcode: () => (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M4 7v10M7 7v10M10 7v4m0 6v-4m3-6v10m3-10v4m0 6v-4m3-6v10" />
    </svg>
  ),
  scan: () => (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
  ...
  onboarding: () => (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M18 9v3m0 4h.01M8 12h2m2 0h2m2 0h2m-4-6h.01M12 6h.01M10 6h.01M12 12v-2m0-4V4m0 0h.01M12 4h.01M10 4h.01M6 8h.01M6 12h.01M6 16h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
    </svg>
  ),
  support: () => (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
  ...
  bot: () => (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M12 1v2m0 18v2m-9-9H1m22 0h-2M5.6 5.6l1.4 1.4m10 10l1.4 1.4M5.6 18.4l1.4-1.4m10-10l1.4-1.4M12 8a4 4 0 100 8 4 4 0 000-8z" />
    </svg>
  ),
  faq: () => (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
...
// Normalizes hyphenated names (e.g., "credit-card" -> "creditcard")
export function getIconComponent(iconName: string): React.ReactNode {
  const normalizedName = iconName.replace(/-/g, '');
  const IconComponent = IconComponents[normalizedName as keyof typeof IconComponents];
...
// NavBadge component
function NavBadge({ text, variant }: { text: string; variant: 'default' | 'success' | 'warning' | 'error' | 'new' }) {
  const variants = {
  ...
  return (
    <span className={cn('ml-auto rounded-full px-2 py-0.5 text-xs font-medium', variants[variant])}>
      {text}
    </span>
  );
}
...
export function NavItemRow({
  ...
  if (hasChildren) {
    ...
    return (
      <div>
        <ToggleLink
          {...(item.href ? { href: item.href, prefetch: item.prefetch ?? true } : { onClick: handleToggle })}
