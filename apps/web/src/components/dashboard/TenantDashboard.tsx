    ...
    const fetchBusinessProfile = async () => {
      try {
        ...
        if (data) {
          ...
          // Also set banner info from profile since it has social_links
          setTenantBanner({
            name: data.business_name || tenantData?.name || 'Store',
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        {/* Banner Hero Section - Show first if available */}
        {tenantBanner?.bannerUrl && (
          <div className="relative w-full rounded-lg overflow-hidden shadow-lg">
            ...
            {tenantBanner.socialLinks && (tenantBanner.socialLinks.facebook || tenantBanner.socialLinks.instagram) && (
              <div className="absolute bottom-3 right-3 flex items-center gap-2">
                {tenantBanner.socialLinks.facebook && (
                  <a
                    href={tenantBanner.socialLinks.facebook}
                    target="_blank"
                ...
                {tenantBanner.socialLinks.instagram && (
                  <a
                    href={tenantBanner.socialLinks.instagram}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center w-9 h-9 rounded-full bg-white/90 hover:bg-white shadow-md transition-all duration-200 hover:scale-110"
                  >
        ...
        {/* Capability-Aware Plan Summary */}
        <PlanSummaryPanel capabilities={allCaps.data} loading={allCaps.loading} tenantId={tenantId} readOnly={!canManageSettings} />
