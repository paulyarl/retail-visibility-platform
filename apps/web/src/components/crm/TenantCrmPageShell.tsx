...
import TenantCrmNavPanel from './TenantCrmNavPanel';

interface TenantCrmPageShellProps {
  tenantId: string;
  title: string;
  subtitle?: ReactNode;
...
export default function TenantCrmPageShell({
  tenantId,
  title,
  subtitle,
  breadcrumbs,
  ...
  children,
  actions,
}: TenantCrmPageShellProps) {
  return (
    <div className="flex flex-col lg:flex-row gap-6">
      {/* Sidebar nav */}
      <aside className="w-full lg:w-56 flex-shrink-0">
        <TenantCrmNavPanel tenantId={tenantId} counts={navCounts} />
      </aside>
