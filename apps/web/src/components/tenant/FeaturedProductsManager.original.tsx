...
import { Search, Star, Sparkles, Eye, ArrowUp, ArrowDown, AlertTriangle, Timer, Clock, Layers, Package, AlertCircle, Calendar, Tag, Award, DollarSign, TrendingUp, Check, ShoppingBag, Download, FileText, Edit2, X, Power, Pause, Play, Edit } from 'lucide-react';
...
const isOutOfStock = (product: FeaturedProduct): boolean => {
...
export default function FeaturedProductsManager({ tenantId }: { tenantId: string }) {
  ...
  const fetchFeaturedLimits = async () => {
    try {
      const response = await apiRequest(`/api/tenant-limits/featured-products?tenantId=${tenantId}`);
      if (response.ok) {
        ...
        console.log('Featured types alignment:', {
          defaults: featuredTypes.map(t => ({ id: t.id, default: t.maxProducts })),
          apiLimits: Object.entries(data.limits).map(([id, limit]) => ({ id, limit })),
          updated: updatedTypes.map(t => ({ id: t.id, final: t.maxProducts }))
        });
  ...
  // Filter active products for the main display (exclude inactive)
  const activeFeatured = currentFeatured.filter(product => product.is_active !== false);
  
  // Filter expired products (by date) AND inactive products
    ...
    try {
      ...
      if (data && data.items) {
        ...
        const availableItems = data.items.filter((item: any) => {
          ...
          // Show all active products that aren't already featured for this type
          // (including out of stock - we'll disable the button for those)
  ...
  // Group products by stock status BEFORE pagination
  const inStockProducts = filteredAvailable.filter(product => !isOutOfStock(product));
  const outOfStockProducts = filteredAvailable.filter(product => isOutOfStock(product));

  // Pagination for each group
  const inStockPageSize = Math.ceil(itemsPerPage * 0.7); // 70% for in-stock
  const outOfStockPageSize = Math.ceil(itemsPerPage * 0.3); // 30% for out-of-stock
  
  const inStockStartIndex = (availablePage - 1) * inStockPageSize;
  const inStockEndIndex = inStockStartIndex + inStockPageSize;
  const paginatedInStock = inStockProducts.slice(inStockStartIndex, inStockEndIndex);
  
  const outOfStockStartIndex = (availablePage - 1) * outOfStockPageSize;
  const outOfStockEndIndex = outOfStockStartIndex + outOfStockPageSize;
  const paginatedOutOfStock = outOfStockProducts.slice(outOfStockStartIndex, outOfStockEndIndex);
  
  // Calculate total pages based on the larger group
  const totalPages = Math.ceil(Math.max(inStockProducts.length, outOfStockProducts.length) / Math.max(inStockPageSize, outOfStockPageSize));
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-gray-700">
              Using <span className="font-semibold">{activeFeatured.length}</span> of{' '}
          ...
          <div className="text-right">
            <div className="text-3xl font-bold text-amber-600">
              {currentType.maxProducts - activeFeatured.length}
            </div>
          ...
          <div className="w-full bg-gray-200 rounded-full h-3">
            <div
              className={`h-3 rounded-full transition-all ${
                activeFeatured.length >= currentType.maxProducts
                  ? 'bg-red-500'
                  : activeFeatured.length / currentType.maxProducts > 0.8
                  ? 'bg-amber-500'
                  : 'bg-green-500'
              }`}
              style={{ width: `${Math.min((activeFeatured.length / currentType.maxProducts) * 100, 100)}%` }}
            ></div>
          </div>
        </div>
      ...
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
          ...
          Currently Featured {currentType.name} ({activeFeatured.length})
        </h2>
      ...
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
          ...
          Inactive {currentType.name} ({expiredFeatured.length})
        </h2>

        {expiredFeatured.length === 0 ? (
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            <Check className="w-16 h-16 text-green-400 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No Inactive {currentType.name}</h3>
            <p className="text-gray-600">
              All your {currentType.name.toLowerCase()} are currently active
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
              <p className="text-sm text-amber-700">
                ⚠️ You have {expiredFeatured.length} inactive {currentType.name.toLowerCase()}. Choose whether to reactivate them or remove them from featuring.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {expiredFeatured.map((product, index) => {
                ...
                <div key={`expired-${product.id || 'unknown'}-${index}`} className="bg-white rounded-lg border-2 border-red-200 p-4 relative opacity-75">
                  ...
                  {/* Renewal Edit Mode */}
                  {editingExpiration === product.id && isExpired && (
      ...
      <div>
        ...
        {paginatedInStock.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              {paginatedInStock.map((product: FeaturedProduct, index: number) => {
            return (
              ...
              <Tooltip
                content={
                  isOutOfStock(product)
                    ? "Cannot add out-of-stock products to featured list. Replenish stock to enable featuring."
                    : processing
                    ? "Processing request..."
                    : currentFeatured.length >= currentType.maxProducts
                    ? "Featured product limit reached for this type."
                    : `Add this product to ${currentType.name}`
                }
              >
                <button
                  onClick={() => {
                  ...
                  disabled={processing || currentFeatured.length >= currentType.maxProducts || isOutOfStock(product)}
        ...
        {paginatedOutOfStock.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              {paginatedOutOfStock.map((product: FeaturedProduct, index: number) => {
            return (
            <div key={`product-outofstock-${product.id || 'unknown'}-${index}`} className="bg-white rounded-lg border border-red-200 p-4 hover:shadow-md transition-shadow opacity-75">
              {/* Copy the same product structure as in-stock products */}
              <div className="flex gap-3">
                {/* Thumbnail */}
                <div className="relative w-16 h-16 bg-gray-100 rounded flex-shrink-0">
                  {product.image_url ? (
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-200">
            <div className="text-sm text-gray-700">
              Showing {paginatedInStock.length + paginatedOutOfStock.length} products ({paginatedInStock.length} in stock, {paginatedOutOfStock.length} out of stock) of {filteredAvailable.length} total
            </div>
            <div className="flex items-center gap-2">
              <button
