...
export default function FeedValidationPage() {
  ...
  useEffect(() => {
    async function fetchValidation() {
      try {
        setLoading(true)
        const res = await api.get(`${API_BASE_URL}/${tenantId}/feed/validate`)
    ...
    async function fetchCoverage() {
      try {
        const r = await api.get(`${API_BASE_URL}/${tenantId}/categories/coverage`)
        if (!r.ok) return
          ...
          <button
            ...
            onClick={async () => {
              try {
                ...
                setError(null)
                // refresh coverage
                try { const r = await api.get(`${API_BASE_URL}/${tenantId}/categories/coverage`); const d = await r.json(); setCoverage(d?.data || null) } catch {}
              } catch (e) {
            ...
            <div className="px-6 py-4 border-t flex gap-3 justify-end">
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    setLoading(true)
                    const res = await api.post(`${API_BASE_URL}/${tenantId}/feed/precheck`)
                    const data = await res.json()
                    const errors: ValidationError[] = []
