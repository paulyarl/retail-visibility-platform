-- Update Materialized Views to Include Store Rating System
...
SELECT 
    ...
    dcl.updated_at,
    dcl.tenant_exists,
    dcl.is_directory_visible,
    dcl.category_ids,
    dcl.category_slugs,
    dcl.store_type_slug,
    dcl.gbp_category_id,
    dcl.gbp_category_name,
    -- Rating fields from summary table
    COALESCE(srs.rating_avg, 0) as rating_avg,
    COALESCE(srs.rating_count, 0) as rating_count,
...
CREATE INDEX IF NOT EXISTS idx_directory_category_listings_category_ids ON directory_category_listings USING GIN(category_ids);
...
WHERE dcl.tenant_exists = true
    AND dcl.is_directory_visible = true
    AND dcl.gbp_category_id IS NOT NULL;

-- Create indexes for performance
CREATE INDEX IF NOT EXISTS idx_directory_gbp_listings_tenant_id ON directory_gbp_listings(tenant_id);
CREATE INDEX IF NOT EXISTS idx_directory_gbp_listings_gbp_category_id ON directory_gbp_listings(gbp_category_id);
...
CREATE INDEX IF NOT EXISTS idx_storefront_mv_category_ids ON storefront_materialized_view USING GIN(category_ids);
...
WITH category_stats AS (
    SELECT 
        ...
        AVG(COALESCE(srs.rating_avg, 0)) as avg_rating,
        SUM(COALESCE(srs.rating_count, 0)) as total_reviews,
    ...
    LEFT JOIN directory_listings_list dcl ON tc.id = ANY(dcl.category_ids)
    LEFT JOIN store_rating_summary srs ON dcl.tenant_id = srs.tenant_id
    WHERE dcl.tenant_exists = true
        AND dcl.is_directory_visible = true
...
WITH store_type_stats AS (
    SELECT 
        ...
        COUNT(CASE WHEN dcl.is_featured = true THEN 1 END) as featured_count,
        COUNT(CASE WHEN dcl.subscription_tier IN ('professional', 'enterprise', 'organization') THEN 1 END) as premium_count,
        COUNT(DISTINCT unnest(dcl.category_ids)) as category_diversity
    FROM directory_listings_list dcl
    LEFT JOIN store_rating_summary srs ON dcl.tenant_id = srs.tenant_id
    WHERE dcl.tenant_exists = true
        AND dcl.is_directory_visible = true
        AND dcl.store_type_slug IS NOT NULL
    GROUP BY dcl.store_type_slug
)
SELECT 
    store_type_slug,
    store_count,
...
-- 8. Create a trigger to automatically refresh materialized views when ratings are updated
CREATE OR REPLACE FUNCTION trigger_refresh_rating_mvs()
RETURNS TRIGGER AS $$
BEGIN
    -- Schedule a refresh (in production, this would be handled by a background job)
    -- For now, we'll just note that a refresh is needed
    RAISE NOTICE 'Materialized views need refresh due to rating changes on tenant %', NEW.tenant_id;
    RETURN NULL;
END;
$$ LANGUAGE plpgsql;

-- Create triggers
DROP TRIGGER IF EXISTS trigger_refresh_rating_mvs_store_reviews ON store_reviews;
CREATE TRIGGER trigger_refresh_rating_mvs_store_reviews
    AFTER INSERT OR UPDATE OR DELETE ON store_reviews
    FOR EACH STATEMENT EXECUTE FUNCTION trigger_refresh_rating_mvs();

DROP TRIGGER IF EXISTS trigger_refresh_rating_mvs_store_rating_summary ON store_rating_summary;
CREATE TRIGGER trigger_refresh_rating_mvs_store_rating_summary
    AFTER INSERT OR UPDATE ON store_rating_summary
    FOR EACH STATEMENT EXECUTE FUNCTION trigger_refresh_rating_mvs();

-- 9. Grant permissions (adjust as needed)
-- GRANT SELECT ON directory_category_listings TO app_user;
-- GRANT SELECT ON directory_gbp_listings TO app_user;
-- GRANT SELECT ON storefront_materialized_view TO app_user;
-- GRANT SELECT ON directory_category_stats TO app_user;
-- GRANT SELECT ON directory_store_type_stats TO app_user;

COMMIT;

-- Usage Notes:
-- 1. Run this script to update all materialized views with rating data
-- 2. The refresh_rating_system() function can be called to update everything
-- 3. In production, schedule refresh_rating_system() to run periodically
-- 4. The triggers will notify when refreshes are needed (implement background jobs)
